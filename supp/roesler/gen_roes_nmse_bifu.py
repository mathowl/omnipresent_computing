...
def unroll_parameter(p_dict):
    ...
    initlen = p_dict['init']
    return N,K,uin,h,mu,sigma,w,F,reg,one_hot,testlen,trainlen,initlen


@nb.njit()
def solve(N,m, x0, p,pvec, Wout,u0, w , ydata,dt):
    

    X = np.zeros((N,m+1),dtype='float64') 

    U_pred = np.zeros((3,400+1),dtype='float64')
    U_pred[:,0] = u0
    Up=u0
...
def test_and_save(N,data_scaled,m, one_hot_output, initlen, trainlen,testlen, h,w,p,reg):
    ...
    ydata = data_scaled[:,::m1][:, initlen+trainlen+1:initlen+trainlen+testlen+1]
...
def main():
    ...
    with shelve.open('data/low_grid.shelve', 'r') as shelf:
        Karray = shelf['K']
        Farray = shelf['F']
        
    N,K,uin,h,mu,sigma,w,F,reg,one_hot_output,testlen,trainlen,initlen = unroll_parameter(p_dict)
