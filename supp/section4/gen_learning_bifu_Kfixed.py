...
def main():
    ...
    for F in np.linspace(0,35,101):
        ...
        Xpred, Upred=test_closed(x0,p,one_hot_output,Wout,w,h,N,60_000) #redo cases between 4-5
        ...
        drsumbool = (0.1<np.sqrt((x0[-10_000:][1:]-x0[-10_000:][:-1])**2+ (y0[-10_000:][1:]-y0[-10_000:][:-1])**2)).sum()
        print(f'{F} batch done')
        with shelve.open(f'shelve/Fbifu11.shelve', 'c') as shelf:
            shelf[str(F)] = [F,K,R,is_per,x0,y0,Xstat,drsumbool,rho0] 
