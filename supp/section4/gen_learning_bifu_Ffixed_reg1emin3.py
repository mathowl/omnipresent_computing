...
def training(data_scaled,p,pvec,w,h,N,x0int, initlen=2500,trainlen=10_000,reg=1e-3):
...
def main():
    ...
    for K in np.linspace(2,20,401):
        F=35
        K=np.round(K,4)

        p = np.array([uin, K,F])
        theta0 = np.linspace(0,2*np.pi,1000)
        x0,Wout,nmse_train = training(data_scaled,p,one_hot_output,w,h,N,theta0,initlen=2_500,trainlen=10_000)
